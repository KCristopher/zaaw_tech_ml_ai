...
Original file is located at
    https://colab.research.google.com/drive/1A9nAxG_uFVW2VhzGKpKT7xWAcsBkSvRT
...
trainset_selected_scaled = pd.DataFrame( scaler.fit_transform(trainset[zmienne_do_modelu]), columns = trainset[zmienne_do_modelu].columns, index=trainset.index )

# na testowym transform a nie fit transform!
testset_selected_scaled = pd.DataFrame( scaler.transform(testset[zmienne_do_modelu]), columns = testset[zmienne_do_modelu].columns, index=testset.index )

trainset_selected_scaled = pd.concat( [trainset_selected_scaled, trainset_selected['MedHouseVal'] ], axis = 1 )
testset_selected_scaled = pd.concat( [testset_selected_scaled, testset_selected['MedHouseVal'] ], axis = 1 )

trainset_selected_scaled.isna().sum()
